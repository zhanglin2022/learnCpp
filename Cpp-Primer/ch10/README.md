...
It truncated. The third parameter is interger. As a result,  all calculation would be converted to integer.
...
```
(a) error: the size of `vec` maybe smaller than `lst`.  
Fixed:  
```cpp
copy(lst.cbegin(), lst.cend(), back_inserter(vec))
```  
  
(b) No error but not any sense. vec.size() still equal zero. Fixed:  
use `fill_n(std::back_inserter(vec), 10, 0)`  
```cpp
fill_n(vec.begin(), 10, 0);
```
...
- `insert` uses `insert`.
>insert function takes a second argument, which must be an iterator into the given container. Elements are inserted ahead of the element denoted by the given iterator.
