...
## Exercise 4.3
>Order of evaluation for most of the binary operators is left undefined to give the compiler opportunities for optimization.This strategy presents a trade-off between efficient code generation
...
Prefix: yields the changed object as its result  
Postfix: yield a copy of the original, unchanged value as its result
...
The character 'q' in the Latin-1 character set has the bit pattern 01110001 (decimal 113, hexadecimal 0x71). Since the machine uses 32-bit integers and 8-bit characters, and 'q' is positive, its representation in a 32-bit integer is 0x00000071.

The bitwise complement ~'q' results in 0xFFFFFF8E (decimal -114 in two's complement representation).

Shifting 0xFFFFFF8E left by 6 bits, which is equivalent to multiplying by 64, -114 x 6 =-7296 (hexadecimal 0xFFFFE380).
 ...
 * The second result is 2 in my compilation environment.

`sizeof(p)` is the size of type `pointer`, and `sizeof(*p)` is the size of type `int`. In my compilation environment, an pointer occupies 8 bytes, and a int occupies 4 bytes. So `sizeof(p)/sizeof(*p)` is 2.
...
yield a copy of the original, unchanged value as its result.
...
we use the loop to through the array ia by the index `ix` and the pointer `ptr`. `ix` and `ptr` have the same function so we can just choose one.
